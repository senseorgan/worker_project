...
class QtLibs:
    ...
    def center_on_screen(inst):
        ...
        inst.move(
            (res.width() / 2) - (inst.frameSize().width() / 2), (res.height() / 2) - (inst.frameSize().height() / 2)
        )
...
class UISettings:
    ...
    def __init__(self, window: typing.Any, ini_fpath: pathlib.Path, cfg_fpath: pathlib.Path):
        ...
        self.__setting_ini = QtCore.QSettings(ini_fpath.as_posix(), QtCore.QSettings.IniFormat)
        self.__setting_cfg: pathlib.Path = cfg_fpath

    def save_main_window_geometry(self):
        self.__setting_ini.setValue(UISettings.KEY_WIN_GEO, self.__window.saveGeometry())
